---
title: "jdepew88/Ralphifier — Claude Code skill discovered by cue"
description: "A playful Ralph Wiggum-inspired text rewriter and installable Claude skill with six levels of Ralphiness."
layout: page
updated: 2026-08-01
tags: [claude-code, skill, core]
---

# ✨ [jdepew88/Ralphifier](https://github.com/jdepew88/Ralphifier)

**★ 0** · strong (score 6) · TypeScript · claude-skill, cloudflare, parody, ralph-wiggum, react

> A playful Ralph Wiggum-inspired text rewriter and installable Claude skill with six levels of Ralphiness.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 6 based on:
- ⭐ 0 stars
- 🗓️ Last pushed: 2026-08-01
- 🏷️ Tags: claude-skill, cloudflare, parody, ralph-wiggum, react, simpsons, text-rewriter, typescript, vite

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add jdepew88/Ralphifier --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/jdepew88/Ralphifier)
