---
title: "ManagementMO/agent2learn — Claude Code skill discovered by cue"
description: "A local, source-cited course vault for AI agents"
layout: page
updated: 2026-09-14
tags: [claude-code, skill, core]
---

# 💎 [ManagementMO/agent2learn](https://github.com/ManagementMO/agent2learn)

**★ 2** · exceptional (score 9) · Python · agent-skill, agent-skills, brightspace, claude-code, codex

> A local, source-cited course vault for AI agents

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 9 based on:
- ⭐ 2 stars
- 🗓️ Last pushed: 2026-09-14
- 🏷️ Tags: agent-skill, agent-skills, brightspace, claude-code, codex, d2l, uwaterloo

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add ManagementMO/agent2learn --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/ManagementMO/agent2learn)
