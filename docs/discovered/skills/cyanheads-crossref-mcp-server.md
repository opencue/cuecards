---
title: "cyanheads/crossref-mcp-server — Claude Code skill discovered by cue"
description: "Resolve DOIs, search ~155M scholarly works, and fetch references via the Crossref REST API. STDIO or Streamable HTTP."
layout: page
updated: 2026-08-07
tags: [claude-code, skill, research, backend]
---

# 💎 [cyanheads/crossref-mcp-server](https://github.com/cyanheads/crossref-mcp-server)

**★ 2** · exceptional (score 8.5) · TypeScript · academic-search, ai-agent, bibliometrics, bun, citation

> Resolve DOIs, search ~155M scholarly works, and fetch references via the Crossref REST API. STDIO or Streamable HTTP.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 8.5 based on:
- ⭐ 2 stars
- 🗓️ Last pushed: 2026-08-07
- 🏷️ Tags: academic-search, ai-agent, bibliometrics, bun, citation, crossref, cyanheads, doi, mcp, mcp-server, model-context-protocol, research, scholarly-metadata, typescript

## Best fit cue profiles

[research](../research.md), [backend](../backend.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add cyanheads/crossref-mcp-server --profile research
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/cyanheads/crossref-mcp-server)
