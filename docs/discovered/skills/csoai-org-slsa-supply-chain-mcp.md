---
title: "CSOAI-ORG/slsa-supply-chain-mcp — Claude Code skill discovered by cue"
description: "SLSA v1.0 supply chain levels MCP. Provenance attestation. MIT"
layout: page
updated: 2026-06-13
tags: [claude-code, skill, core]
---

# ✨ [CSOAI-ORG/slsa-supply-chain-mcp](https://github.com/CSOAI-ORG/slsa-supply-chain-mcp)

**★ 0** · strong (score 6) · Python · ai-agents, ai-compliance, ai-security, claude, mcp

> SLSA v1.0 supply chain levels MCP. Provenance attestation. MIT

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 6 based on:
- ⭐ 0 stars
- 🗓️ Last pushed: 2026-06-13
- 🏷️ Tags: ai-agents, ai-compliance, ai-security, claude, mcp, mcp-server, meok, meok-ai-labs, mit-license, model-context-protocol, open-source, provenance, security, slsa, supply-chain

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add CSOAI-ORG/slsa-supply-chain-mcp --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/CSOAI-ORG/slsa-supply-chain-mcp)
