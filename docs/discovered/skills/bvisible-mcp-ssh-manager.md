---
title: "bvisible/mcp-ssh-manager — Claude Code skill discovered by cue"
description: "MCP SSH Server: 37 tools for remote SSH management | Claude Code & OpenAI Codex | DevOps automation, backups, database operations, health monitoring"
layout: page
updated: 2026-06-30
tags: [claude-code, skill, backend]
---

# 💎 [bvisible/mcp-ssh-manager](https://github.com/bvisible/mcp-ssh-manager)

**★ 332** · exceptional (score 17.4) · JavaScript · anthropic, automation, backup, claude-code, database

> MCP SSH Server: 37 tools for remote SSH management | Claude Code & OpenAI Codex | DevOps automation, backups, database operations, health monitoring

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 17.4 based on:
- ⭐ 332 stars
- 🗓️ Last pushed: 2026-06-30
- 🏷️ Tags: anthropic, automation, backup, claude-code, database, deployment, devops, erpnext, frappe, mcp, mcp-server, mcp-ssh, model-context-protocol, monitoring, openai-codex, remote-server, ssh, ssh-manager, ssh-mcp, toml

## Best fit cue profiles

[backend](../backend.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add bvisible/mcp-ssh-manager --profile backend
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/bvisible/mcp-ssh-manager)
