---
title: "DataCore-VietNam/datacore-a2a-mcp — Claude Code skill discovered by cue"
description: "MCP server for DataCore A2A — Vietnamese address normalisation and company lookup as native MCP tools"
layout: page
updated: 2026-08-26
tags: [claude-code, skill, core]
---

# ✨ [DataCore-VietNam/datacore-a2a-mcp](https://github.com/DataCore-VietNam/datacore-a2a-mcp)

**★ 0** · strong (score 6) · Python · a2a, address-normalization, company-data, mcp, mcp-server

> MCP server for DataCore A2A — Vietnamese address normalisation and company lookup as native MCP tools

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 6 based on:
- ⭐ 0 stars
- 🗓️ Last pushed: 2026-08-26
- 🏷️ Tags: a2a, address-normalization, company-data, mcp, mcp-server, model-context-protocol, python, vietnam

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add DataCore-VietNam/datacore-a2a-mcp --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/DataCore-VietNam/datacore-a2a-mcp)
