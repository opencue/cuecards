---
title: "ManSio/mscodebase-intelligence — Claude Code skill discovered by cue"
description: "Intelligent codebase search & indexing for Zed. Async MCP server featuring LanceDB/BM25 hybrid search, multi-bucket RAG, and autonomous self-healing workflows. "
layout: page
updated: 2026-08-14
tags: [claude-code, skill, core]
---

# ✨ [ManSio/mscodebase-intelligence](https://github.com/ManSio/mscodebase-intelligence)

**★ 2** · strong (score 5.5) · Python · adr, auto-intent-detection, call-graph, codebase, codebase-intelligence

> Intelligent codebase search & indexing for Zed. Async MCP server featuring LanceDB/BM25 hybrid search, multi-bucket RAG, and autonomous self-healing workflows. High-performance, memory-safe, and ready for your production code.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 5.5 based on:
- ⭐ 2 stars
- 🗓️ Last pushed: 2026-08-14
- 🏷️ Tags: adr, auto-intent-detection, call-graph, codebase, codebase-intelligence, context-server, hybrid-search, index, lancedb, llm-context, mcp-server, python-asyncio, rag, semantic, semantic-search, windows-development, zed-editor

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add ManSio/mscodebase-intelligence --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/ManSio/mscodebase-intelligence)
