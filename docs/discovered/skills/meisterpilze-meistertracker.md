---
title: "meisterpilze/meistertracker — Claude Code skill discovered by cue"
description: "Self-hosted lab management for specialty mushroom cultivation — barcode scanning, batch / culture / harvest tracking, label printing, CalDAV sync, MCP integrati"
layout: page
updated: 2026-09-24
tags: [claude-code, skill, backend]
---

# 💎 [meisterpilze/meistertracker](https://github.com/meisterpilze/meistertracker)

**★ 3** · exceptional (score 8.2) · JavaScript · agpl, barcode-scanner, caldav, fungi, inventory-tracking

> Self-hosted lab management for specialty mushroom cultivation — barcode scanning, batch / culture / harvest tracking, label printing, CalDAV sync, MCP integration. Built at meisterpilze.de.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 8.2 based on:
- ⭐ 3 stars
- 🗓️ Last pushed: 2026-09-24
- 🏷️ Tags: agpl, barcode-scanner, caldav, fungi, inventory-tracking, lab-management, mcp-server, model-context-protocol, mushroom, mushroom-cultivation, mushroom-farm, mycology, nodejs, pilzzucht, pwa, self-hosted, specialty-mushrooms, sqlite, zebra-printer

## Best fit cue profiles

[backend](../backend.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add meisterpilze/meistertracker --profile backend
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/meisterpilze/meistertracker)
