---
title: "ruialexrib/denarius-ai — Claude Code skill discovered by cue"
description: "Personal and family finance management platform with double-entry accounting, budgeting, reconciliation, analytics, savings tracking and AI-powered financial in"
layout: page
updated: 2026-09-01
tags: [claude-code, skill, backend]
---

# 💎 [ruialexrib/denarius-ai](https://github.com/ruialexrib/denarius-ai)

**★ 0** · exceptional (score 8) · C# · ai-agent, csharp, docker, mcp-server, mistral

> Personal and family finance management platform with double-entry accounting, budgeting, reconciliation, analytics, savings tracking and AI-powered financial insights.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 8 based on:
- ⭐ 0 stars
- 🗓️ Last pushed: 2026-09-01
- 🏷️ Tags: ai-agent, csharp, docker, mcp-server, mistral, mvc, netcore9, personal-finance, sql-server

## Best fit cue profiles

[backend](../backend.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add ruialexrib/denarius-ai --profile backend
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/ruialexrib/denarius-ai)
