---
title: "samarthanalytics-sj/samarth-analytics-mcp — Claude Code skill discovered by cue"
description: "MCP server for Samarth Analytics Google Tag Manager operations"
layout: page
updated: 2026-09-22
tags: [claude-code, skill, backend]
---

# 💎 [samarthanalytics-sj/samarth-analytics-mcp](https://github.com/samarthanalytics-sj/samarth-analytics-mcp)

**★ 2** · exceptional (score 10) · TypeScript · ai-agent, consent-mode, consent-mode-v2, cookie-consent, ga4

> MCP server for Samarth Analytics Google Tag Manager operations

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 10 based on:
- ⭐ 2 stars
- 🗓️ Last pushed: 2026-09-22
- 🏷️ Tags: ai-agent, consent-mode, consent-mode-v2, cookie-consent, ga4, ga4-admin-api, ga4-data-api, google-ads, google-analytics, google-tag-manager, google-tag-manager-api, gtm, mcp, mcp-server, model-context-protocol, playwright, server-side-tagging, tag-auditing, typescript, web-audit

## Best fit cue profiles

[backend](../backend.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add samarthanalytics-sj/samarth-analytics-mcp --profile backend
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/samarthanalytics-sj/samarth-analytics-mcp)
