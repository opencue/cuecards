---
title: "Thepizzapie/BuildersGate — Claude Code skill discovered by cue"
description: "Agentic game development for building games with AI Agents. Run one session per seat: art, gameplay, narrative, QA, audio, tech all in 1 spot"
layout: page
updated: 2026-08-20
tags: [claude-code, skill, core]
---

# 💎 [Thepizzapie/BuildersGate](https://github.com/Thepizzapie/BuildersGate)

**★ 21** · exceptional (score 13) · Python · agent-framework, agent-memory, agent-orchestration, agent-skill, claude

> Agentic game development for building games with AI Agents. Run one session per seat: art, gameplay, narrative, QA, audio, tech all in 1 spot

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 13 based on:
- ⭐ 21 stars
- 🗓️ Last pushed: 2026-08-20
- 🏷️ Tags: agent-framework, agent-memory, agent-orchestration, agent-skill, claude, claude-code, game-development, godot, godot-engine, mcp, mcp-server

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add Thepizzapie/BuildersGate --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/Thepizzapie/BuildersGate)
