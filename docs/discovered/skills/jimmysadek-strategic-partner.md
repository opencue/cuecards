---
title: "JimmySadek/strategic-partner — Claude Code skill discovered by cue"
description: "Every other tool executes. This one decides what to execute. Chief of Staff skill for Claude Code."
layout: page
updated: 2026-08-08
tags: [claude-code, skill, core]
---

# 💎 [JimmySadek/strategic-partner](https://github.com/JimmySadek/strategic-partner)

**★ 24** · exceptional (score 16.5) · Shell · advisory, ai-skill, claude-code, claude-code-skill, cross-agent

> Every other tool executes. This one decides what to execute. Chief of Staff skill for Claude Code.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 16.5 based on:
- ✅ Contains SKILL.md
- ⭐ 24 stars
- 🗓️ Last pushed: 2026-08-08
- 🏷️ Tags: advisory, ai-skill, claude-code, claude-code-skill, cross-agent, prompt-engineering, skillshare

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add JimmySadek/strategic-partner --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/JimmySadek/strategic-partner)
