---
title: "lainXXX/blog-auto-poster-skill — Claude Code skill discovered by cue"
description: "多平台文章统一发布 Skill — 支持 CSDN、51CTO、博客园、掘金一键分发"
layout: page
updated: 2026-07-01
tags: [claude-code, skill, core]
---

# 💎 [lainXXX/blog-auto-poster-skill](https://github.com/lainXXX/blog-auto-poster-skill)

**★ 12** · exceptional (score 12.3) · Python · 51cto, automation, blog-poster, claude-code-skill, cnblogs

> 多平台文章统一发布 Skill — 支持 CSDN、51CTO、博客园、掘金一键分发

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 12.3 based on:
- ✅ Contains SKILL.md
- ⭐ 12 stars
- 🗓️ Last pushed: 2026-07-01
- 🏷️ Tags: 51cto, automation, blog-poster, claude-code-skill, cnblogs, csdn, jianshu, juejin, multi-platform, python

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add lainXXX/blog-auto-poster-skill --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/lainXXX/blog-auto-poster-skill)
