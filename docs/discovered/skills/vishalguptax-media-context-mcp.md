---
title: "vishalguptax/media-context-mcp — Claude Code skill discovered by cue"
description: "Local MCP server to analyze video, audio & images — extract frames, transcribe speech, OCR on-screen text. Give Claude, Cursor & VS Code video context. No API k"
layout: page
updated: 2026-06-24
tags: [claude-code, skill, video, creative-media]
---

# ✨ [vishalguptax/media-context-mcp](https://github.com/vishalguptax/media-context-mcp)

**★ 0** · strong (score 6) · TypeScript · ai-agents, audio, claude, cursor, ffmpeg

> Local MCP server to analyze video, audio & images — extract frames, transcribe speech, OCR on-screen text. Give Claude, Cursor & VS Code video context. No API keys, no cloud.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 6 based on:
- ⭐ 0 stars
- 🗓️ Last pushed: 2026-06-24
- 🏷️ Tags: ai-agents, audio, claude, cursor, ffmpeg, mcp, mcp-server, model-context-protocol, multimodal, ocr, screen-recording, speech-to-text, tesseract, transcription, video, video-analysis, video-to-text, whisper, yt-dlp

## Best fit cue profiles

[video](../video.md), [creative-media](../creative-media.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add vishalguptax/media-context-mcp --profile video
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/vishalguptax/media-context-mcp)
