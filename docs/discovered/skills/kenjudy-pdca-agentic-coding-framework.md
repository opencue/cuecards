---
title: "kenjudy/pdca-agentic-coding-framework — Claude Code skill discovered by cue"
description: "Human-supervised AI code generation using Plan-Do-Check-Act methodology with TDD and refactoring. Works as Claude Code skill or standalone prompts."
layout: page
updated: 2026-08-18
tags: [claude-code, skill, core]
---

# 💎 [kenjudy/pdca-agentic-coding-framework](https://github.com/kenjudy/pdca-agentic-coding-framework)

**★ 41** · exceptional (score 10.9) · Python · ai-coding, anthropic, claude, claude-skill, code-generation

> Human-supervised AI code generation using Plan-Do-Check-Act methodology with TDD and refactoring. Works as Claude Code skill or standalone prompts.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 10.9 based on:
- ⭐ 41 stars
- 🗓️ Last pushed: 2026-08-18
- 🏷️ Tags: ai-coding, anthropic, claude, claude-skill, code-generation, human-in-the-loop, pair-programming, pdca, tdd

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add kenjudy/pdca-agentic-coding-framework --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/kenjudy/pdca-agentic-coding-framework)
