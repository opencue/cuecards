---
title: "dirmacs/daedra — Claude Code skill discovered by cue"
description: "Self-contained web search MCP server. Multiple backends with automatic fallback. Pure Rust. Works from any IP."
layout: page
updated: 2026-08-29
tags: [claude-code, skill, core]
---

# 💎 [dirmacs/daedra](https://github.com/dirmacs/daedra)

**★ 13** · exceptional (score 10.2) · Rust · ai, ddg, duckduckgo, json-rpc, mcp

> Self-contained web search MCP server. Multiple backends with automatic fallback. Pure Rust. Works from any IP.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 10.2 based on:
- ⭐ 13 stars
- 🗓️ Last pushed: 2026-08-29
- 🏷️ Tags: ai, ddg, duckduckgo, json-rpc, mcp, mcp-server, model-context-protocol, rust, search, tool-calling, web-search

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add dirmacs/daedra --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/dirmacs/daedra)
