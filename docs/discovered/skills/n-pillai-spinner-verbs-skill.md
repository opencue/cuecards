---
title: "n-pillai/spinner-verbs-skill — Claude Code skill discovered by cue"
description: "A Claude Code skill for creating deeply personalized spinner verbs based on your life, interests, and personality"
layout: page
updated: 2026-08-18
tags: [claude-code, skill, core]
---

# 💎 [n-pillai/spinner-verbs-skill](https://github.com/n-pillai/spinner-verbs-skill)

**★ 2** · exceptional (score 15) · claude-code, claude-skill, customization, developer-tools, personalization

> A Claude Code skill for creating deeply personalized spinner verbs based on your life, interests, and personality

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 15 based on:
- ✅ Contains SKILL.md
- ⭐ 2 stars
- 🗓️ Last pushed: 2026-08-18
- 🏷️ Tags: claude-code, claude-skill, customization, developer-tools, personalization, terminal

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add n-pillai/spinner-verbs-skill --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/n-pillai/spinner-verbs-skill)
