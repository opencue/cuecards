---
title: "buildkite/buildkite-mcp-server — Claude Code skill discovered by cue"
description: "Official MCP Server for Buildkite."
layout: page
updated: 2026-07-21
tags: [claude-code, skill, core]
---

# 💎 [buildkite/buildkite-mcp-server](https://github.com/buildkite/buildkite-mcp-server)

**★ 52** · exceptional (score 14.1) · Go · buildkite, mcp-server

> Official MCP Server for Buildkite.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 14.1 based on:
- ⭐ 52 stars
- 🗓️ Last pushed: 2026-07-21
- 🏷️ Tags: buildkite, mcp-server

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add buildkite/buildkite-mcp-server --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/buildkite/buildkite-mcp-server)
