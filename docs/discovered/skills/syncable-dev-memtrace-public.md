---
title: "syncable-dev/memtrace-public — Claude Code skill discovered by cue"
description: "Structural memory for AI coding agents. Bi-temporal graph, MCP-native, zero LLM calls. Cursor · Claude Code · Codex · Hermes · VS Code · Windsurf."
layout: page
updated: 2026-07-09
tags: [claude-code, skill, core]
---

# 💎 [syncable-dev/memtrace-public](https://github.com/syncable-dev/memtrace-public)

**★ 364** · exceptional (score 14.7) · Python · agent-memory, ai-agents, bi-temporal-graph, claude-code, code-intelligence

> Structural memory for AI coding agents. Bi-temporal graph, MCP-native, zero LLM calls. Cursor · Claude Code · Codex · Hermes · VS Code · Windsurf.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 14.7 based on:
- ⭐ 364 stars
- 🗓️ Last pushed: 2026-07-09
- 🏷️ Tags: agent-memory, ai-agents, bi-temporal-graph, claude-code, code-intelligence, code-search, coding-agents, cursor, developer-tools, knowledge-graph, llm, local-first, mcp, mcp-server, rag, rust, semantic-search, structural-memory, temporal-analysis, tree-sitter

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add syncable-dev/memtrace-public --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/syncable-dev/memtrace-public)
