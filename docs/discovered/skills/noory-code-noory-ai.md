---
title: "noory-code/noory-ai — Claude Code skill discovered by cue"
description: "Claude Code plugins and MCP servers — Evonest, Distill, Solera, metang, and more"
layout: page
updated: 2026-08-06
tags: [claude-code, skill, core]
---

# 💎 [noory-code/noory-ai](https://github.com/noory-code/noory-ai)

**★ 0** · exceptional (score 10) · Python · ai-agents, claude-code, claude-plugin, mcp-server, model-context-protocol

> Claude Code plugins and MCP servers — Evonest, Distill, Solera, metang, and more

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 10 based on:
- ⭐ 0 stars
- 🗓️ Last pushed: 2026-08-06
- 🏷️ Tags: ai-agents, claude-code, claude-plugin, mcp-server, model-context-protocol, python

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add noory-code/noory-ai --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/noory-code/noory-ai)
