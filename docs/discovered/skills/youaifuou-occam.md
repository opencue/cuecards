---
title: "youaifuou/occam — Claude Code skill discovered by cue"
description: "An explicitly invoked Agent Skill for complete work and minimal sufficient communication."
layout: page
updated: 2026-07-24
tags: [claude-code, skill, core]
---

# ✨ [youaifuou/occam](https://github.com/youaifuou/occam)

**★ 1** · strong (score 6.3) · agent-skills, claude-code-skill, claude-skills

> An explicitly invoked Agent Skill for complete work and minimal sufficient communication.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 6.3 based on:
- ⭐ 1 stars
- 🗓️ Last pushed: 2026-07-24
- 🏷️ Tags: agent-skills, claude-code-skill, claude-skills

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add youaifuou/occam --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/youaifuou/occam)
