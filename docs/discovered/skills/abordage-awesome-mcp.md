---
title: "abordage/awesome-mcp — Claude Code skill discovered by cue"
description: "Curated list of Model Context Protocol (MCP) servers, clients, and frameworks. Automatically updated daily."
layout: page
updated: 2026-06-17
tags: [claude-code, skill, core]
---

# 💎 [abordage/awesome-mcp](https://github.com/abordage/awesome-mcp)

**★ 8** · exceptional (score 10.8) · awesome-list, mcp, mcp-client, mcp-server, mcp-tools

> Curated list of Model Context Protocol (MCP) servers, clients, and frameworks. Automatically updated daily.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 10.8 based on:
- ⭐ 8 stars
- 🗓️ Last pushed: 2026-06-17
- 🏷️ Tags: awesome-list, mcp, mcp-client, mcp-server, mcp-tools, model-context-protocol

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add abordage/awesome-mcp --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/abordage/awesome-mcp)
