---
title: "thdelmas/consciousness-loop — Claude Code skill discovered by cue"
description: "Consciousness Loop — the self-firing executive of an agent: wake → integrate the organs → decide → act → set its own frequency → sleep. The keystone of agent-ne"
layout: page
updated: 2026-09-21
tags: [claude-code, skill, core]
---

# 💎 [thdelmas/consciousness-loop](https://github.com/thdelmas/consciousness-loop)

**★ 4** · exceptional (score 16.8) · Python · agent-loop, agent-nervous-system, agent-skills, ai-agent, anthropic

> Consciousness Loop — the self-firing executive of an agent: wake → integrate the organs → decide → act → set its own frequency → sleep. The keystone of agent-nervous-system. Functional, not phenomenal.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 16.8 based on:
- ✅ Contains SKILL.md
- ⭐ 4 stars
- 🗓️ Last pushed: 2026-09-21
- 🏷️ Tags: agent-loop, agent-nervous-system, agent-skills, ai-agent, anthropic, autonomous-agents, claude-code, claude-skill, codex, cognitive-architecture, cursor, developer-tools, global-workspace, llm, scheduling

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add thdelmas/consciousness-loop --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/thdelmas/consciousness-loop)
