---
title: "110kc3/seo — Claude Code skill discovered by cue"
description: "Open registry and discovery API for AI products, agent tools, and MCP services."
layout: page
updated: 2026-09-14
tags: [claude-code, skill, backend]
---

# ✨ [110kc3/seo](https://github.com/110kc3/seo)

**★ 1** · strong (score 6.3) · JavaScript · ai-agents, ai-discovery, directory, llms-txt, mcp

> Open registry and discovery API for AI products, agent tools, and MCP services.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 6.3 based on:
- ⭐ 1 stars
- 🗓️ Last pushed: 2026-09-14
- 🏷️ Tags: ai-agents, ai-discovery, directory, llms-txt, mcp, mcp-server, schema-org, seo

## Best fit cue profiles

[backend](../backend.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add 110kc3/seo --profile backend
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/110kc3/seo)
