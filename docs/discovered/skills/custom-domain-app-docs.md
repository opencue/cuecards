---
title: "CUSTOM-DOMAIN-APP/docs — Claude Code skill discovered by cue"
description: "Docs for customdomain.ai — custom domains for SaaS. Let users connect their own domain in one click: automatic DNS configuration, CNAME/TXT domain verification,"
layout: page
updated: 2026-08-15
tags: [claude-code, skill, docs-writer, backend]
---

# ✨ [CUSTOM-DOMAIN-APP/docs](https://github.com/CUSTOM-DOMAIN-APP/docs)

**★ 3** · strong (score 5.7) · MDX · ai-agents, cname, custom-domain, custom-domains, custom-hostname

> Docs for customdomain.ai — custom domains for SaaS. Let users connect their own domain in one click: automatic DNS configuration, CNAME/TXT domain verification, auto-issued SSL/TLS certificates. Guides + reference for the embeddable widget, REST API, and hosted MCP server for AI agents. Entri alternative.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 5.7 based on:
- ⭐ 3 stars
- 🗓️ Last pushed: 2026-08-15
- 🏷️ Tags: ai-agents, cname, custom-domain, custom-domains, custom-hostname, dns, dns-automation, docs, documentation, domain-verification, letsencrypt, mcp-server, multi-tenant, rest-api, saas, ssl, tls, white-label

## Best fit cue profiles

[docs-writer](../docs-writer.md), [backend](../backend.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add CUSTOM-DOMAIN-APP/docs --profile docs-writer
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/CUSTOM-DOMAIN-APP/docs)
