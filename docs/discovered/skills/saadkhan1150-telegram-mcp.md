---
title: "saadkhan1150/telegram-mcp — Claude Code skill discovered by cue"
description: "🤖 Manage multiple Telegram accounts effortlessly with AI-driven tools for bulk messaging, scheduling, and more in one easy-to-use platform."
layout: page
updated: 2026-07-06
tags: [claude-code, skill, core]
---

# 💎 [saadkhan1150/telegram-mcp](https://github.com/saadkhan1150/telegram-mcp)

**★ 9** · exceptional (score 9.9) · Python · admin, ai, automation, bark, bot

> 🤖 Manage multiple Telegram accounts effortlessly with AI-driven tools for bulk messaging, scheduling, and more in one easy-to-use platform.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 9.9 based on:
- ⭐ 9 stars
- 🗓️ Last pushed: 2026-07-06
- 🏷️ Tags: admin, ai, automation, bark, bot, chat-management, cursor-ai, data-analysis, fastmcp, mcp-server, messaging, model-context-protocol, mtproto, python, search

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add saadkhan1150/telegram-mcp --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/saadkhan1150/telegram-mcp)
