---
title: "Terryc21/bug-echo — Claude Code skill discovered by cue"
description: "Claude Code skill that finds and rates other instances of a bug you just fixed, then presents options to fix them after explicit user approval"
layout: page
updated: 2026-08-27
tags: [claude-code, skill, core]
---

# 💎 [Terryc21/bug-echo](https://github.com/Terryc21/bug-echo)

**★ 21** · exceptional (score 11.4) · Python · bug-hunting, claude-code, claude-code-skill, debugging

> Claude Code skill that finds and rates other instances of a bug you just fixed, then presents options to fix them after explicit user approval

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 11.4 based on:
- ⭐ 21 stars
- 🗓️ Last pushed: 2026-08-27
- 🏷️ Tags: bug-hunting, claude-code, claude-code-skill, debugging

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add Terryc21/bug-echo --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/Terryc21/bug-echo)
