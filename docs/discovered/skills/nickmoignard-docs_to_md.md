---
title: "NickMoignard/docs_to_md — Claude Code skill discovered by cue"
description: "A Claude Code skill + Python crawler that mirror a product's documentation site into local markdown for agent consumption"
layout: page
updated: 2026-06-28
tags: [claude-code, skill, docs-writer]
---

# 💎 [NickMoignard/docs_to_md](https://github.com/NickMoignard/docs_to_md)

**★ 0** · exceptional (score 13) · Python · claude-code, claude-skill, documentation, llm, markdown

> A Claude Code skill + Python crawler that mirror a product's documentation site into local markdown for agent consumption

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 13 based on:
- ✅ Contains SKILL.md
- ⭐ 0 stars
- 🗓️ Last pushed: 2026-06-28
- 🏷️ Tags: claude-code, claude-skill, documentation, llm, markdown, python, web-crawler

## Best fit cue profiles

[docs-writer](../docs-writer.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add NickMoignard/docs_to_md --profile docs-writer
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/NickMoignard/docs_to_md)
