---
title: "zhouyuanxinand/nocap — Claude Code skill discovered by cue"
description: "🧢→✅ A lie detector for AI coding agents — runs the receipt, catches \"done\" lies"
layout: page
updated: 2026-08-13
tags: [claude-code, skill, core]
---

# ✨ [zhouyuanxinand/nocap](https://github.com/zhouyuanxinand/nocap)

**★ 0** · strong (score 5.6) · TypeScript · ai-agent, ai-agent-framework, ai-agent-security, ai-agent-skill, ai-agent-skills

> 🧢→✅ A lie detector for AI coding agents — runs the receipt, catches "done" lies

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 5.6 based on:
- ⭐ 0 stars
- 🗓️ Last pushed: 2026-08-04
- 🏷️ Tags: ai-agent, ai-agent-framework, ai-agent-security, ai-agent-skill, ai-agent-skills, ai-agent-tools, ai-agents, ai-agents-automation, ai-agents-framework, ai-agents-platform

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add zhouyuanxinand/nocap --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/zhouyuanxinand/nocap)
