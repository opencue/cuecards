---
title: "IdoCohen560/claude-unity-game-studio — Claude Code skill discovered by cue"
description: "AI-powered Unity game development studio for Claude Code - 49 agents, 120+ skills, and a full studio hierarchy that turns a single AI   session into a coordinat"
layout: page
updated: 2026-07-08
tags: [claude-code, skill, core]
---

# 💎 [IdoCohen560/claude-unity-game-studio](https://github.com/IdoCohen560/claude-unity-game-studio)

**★ 7** · exceptional (score 11.7) · C# · claude, claude-ai, claude-code, claude-code-plugin, claude-code-skill

> AI-powered Unity game development studio for Claude Code - 49 agents, 120+ skills, and a full studio hierarchy that turns a single AI   session into a coordinated game dev team. View more about me in my portfolio.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 11.7 based on:
- ⭐ 7 stars
- 🗓️ Last pushed: 2026-07-08
- 🏷️ Tags: claude, claude-ai, claude-code, claude-code-plugin, claude-code-skill, claude-code-skills, claude-skill, claude-skills, gaming, gaming-community, gaming-software, gaming-tools

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add IdoCohen560/claude-unity-game-studio --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/IdoCohen560/claude-unity-game-studio)
