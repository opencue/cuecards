---
title: "Anakior/atlas-mind — Claude Code skill discovered by cue"
description: "Self-hostable, git-native knowledge base & AI memory (MCP) — your notes stay plain Markdown files in your own git repo. Pure-stdlib Python, no database. AGPL-3."
layout: page
updated: 2026-06-29
tags: [claude-code, skill, docs-writer, backend]
---

# ✨ [Anakior/atlas-mind](https://github.com/Anakior/atlas-mind)

**★ 2** · strong (score 6.5) · Python · ai, claude, digital-garden, git, knowledge-base

> Self-hostable, git-native knowledge base & AI memory (MCP) — your notes stay plain Markdown files in your own git repo. Pure-stdlib Python, no database. AGPL-3.0.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 6.5 based on:
- ⭐ 2 stars
- 🗓️ Last pushed: 2026-06-29
- 🏷️ Tags: ai, claude, digital-garden, git, knowledge-base, knowledge-management, markdown, mcp, mcp-server, model-context-protocol, note-taking, notes, obsidian, pkm, python, second-brain, self-hosted, self-hosting, wiki, zettelkasten

## Best fit cue profiles

[docs-writer](../docs-writer.md), [backend](../backend.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add Anakior/atlas-mind --profile docs-writer
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/Anakior/atlas-mind)
