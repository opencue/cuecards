---
title: "trudenboy/ma-provider-mcp — Claude Code skill discovered by cue"
description: "FastMCP server for Music Assistant — control MA from Claude Code, Codex, Cursor, and other AI agents"
layout: page
updated: 2026-07-14
tags: [claude-code, skill, core]
---

# 💎 [trudenboy/ma-provider-mcp](https://github.com/trudenboy/ma-provider-mcp)

**★ 1** · exceptional (score 8.8) · Python · ai, ai-agents, chatgpt, claude-code, cline

> FastMCP server for Music Assistant — control MA from Claude Code, Codex, Cursor, and other AI agents

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 8.8 based on:
- ⭐ 1 stars
- 🗓️ Last pushed: 2026-07-14
- 🏷️ Tags: ai, ai-agents, chatgpt, claude-code, cline, codex, cursor, fastmcp, gemini, home-assistant, llm-tools, mcp, mcp-server, music-assistant, plugin-provider, python, vscode, zed

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add trudenboy/ma-provider-mcp --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/trudenboy/ma-provider-mcp)
