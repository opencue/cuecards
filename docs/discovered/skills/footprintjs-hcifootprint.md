---
title: "footprintjs/hcifootprint — Claude Code skill discovered by cue"
description: "Turn a web app's interaction surface into a typed, traversable skill graph an LLM can plan over — the frontend sibling of footprintjs."
layout: page
updated: 2026-08-20
tags: [claude-code, skill, core]
---

# 💎 [footprintjs/hcifootprint](https://github.com/footprintjs/hcifootprint)

**★ 9** · exceptional (score 8.2) · TypeScript · agentic-apps, footprintjs, hci, llm-agents, mcp

> Turn a web app's interaction surface into a typed, traversable skill graph an LLM can plan over — the frontend sibling of footprintjs.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 8.2 based on:
- ⭐ 9 stars
- 🗓️ Last pushed: 2026-08-20
- 🏷️ Tags: agentic-apps, footprintjs, hci, llm-agents, mcp, mcp-server, skill-graph, typescript

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add footprintjs/hcifootprint --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/footprintjs/hcifootprint)
