---
title: "alex-jb/shadow-mentor — Claude Code skill discovered by cue"
description: "Shadow — on-device AI council and audit chain for regulated banking workflows. One engine, 4 device clients, 5 persona packs."
layout: page
updated: 2026-07-08
tags: [claude-code, skill, core]
---

# 💎 [alex-jb/shadow-mentor](https://github.com/alex-jb/shadow-mentor)

**★ 0** · exceptional (score 15.5) · JavaScript · agent-skill, banking, brier-calibration, claude-skill, compliance

> Shadow — on-device AI council and audit chain for regulated banking workflows. One engine, 4 device clients, 5 persona packs.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 15.5 based on:
- ✅ Contains SKILL.md
- ⭐ 0 stars
- 🗓️ Last pushed: 2026-07-08
- 🏷️ Tags: agent-skill, banking, brier-calibration, claude-skill, compliance, fintech, loan-origination, mcp, mcp-server, skills-sh

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add alex-jb/shadow-mentor --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/alex-jb/shadow-mentor)
