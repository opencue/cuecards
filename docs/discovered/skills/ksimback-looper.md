---
title: "ksimback/looper — Claude Code skill discovered by cue"
description: "Design visual, review-gated agent loops for Claude Code before you run them."
layout: page
updated: 2026-07-05
tags: [claude-code, skill, core]
---

# 💎 [ksimback/looper](https://github.com/ksimback/looper)

**★ 577** · exceptional (score 20.5) · Python · agent-loops, agentic-workflows, ai-agents, claude-code, claude-code-skill

> Design visual, review-gated agent loops for Claude Code before you run them.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 20.5 based on:
- ✅ Contains SKILL.md
- ⭐ 577 stars
- 🗓️ Last pushed: 2026-07-05
- 🏷️ Tags: agent-loops, agentic-workflows, ai-agents, claude-code, claude-code-skill, llm-as-judge

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add ksimback/looper --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/ksimback/looper)
