---
title: "millsymills-com/flipperzero-mcp — Claude Code skill discovered by cue"
description: "Modular MCP server for the Flipper Zero (USB + WiFi protobuf RPC)"
layout: page
updated: 2026-06-09
tags: [claude-code, skill, core]
---

# ✨ [millsymills-com/flipperzero-mcp](https://github.com/millsymills-com/flipperzero-mcp)

**★ 2** · strong (score 6.5) · Python · flipper-zero, hardware, mcp, mcp-server, model-context-protocol

> Modular MCP server for the Flipper Zero (USB + WiFi protobuf RPC)

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 6.5 based on:
- ⭐ 2 stars
- 🗓️ Last pushed: 2026-06-09
- 🏷️ Tags: flipper-zero, hardware, mcp, mcp-server, model-context-protocol, protobuf, python, rpc, usb

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add millsymills-com/flipperzero-mcp --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/millsymills-com/flipperzero-mcp)
