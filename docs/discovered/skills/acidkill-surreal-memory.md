---
title: "acidkill/surreal-memory — Claude Code skill discovered by cue"
description: "Claude Code skill from acidkill/surreal-memory"
layout: page
updated: 2026-09-24
tags: [claude-code, skill, core]
---

# 💎 [acidkill/surreal-memory](https://github.com/acidkill/surreal-memory)

**★ 7** · exceptional (score 11.2) · Python · agent-memory, ai-agents, claude-code, cursor, knowledge-graph

> A Claude Code skill repository discovered by cue.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 11.2 based on:
- ⭐ 7 stars
- 🗓️ Last pushed: 2026-09-24
- 🏷️ Tags: agent-memory, ai-agents, claude-code, cursor, knowledge-graph, mcp, mcp-server, python, rag, surrealdb, temporal, vector-search

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add acidkill/surreal-memory --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/acidkill/surreal-memory)
