---
title: "camwilso/openboard — Claude Code skill discovered by cue"
description: "Mac App to use Codex Micro or Creator Micro with Claude, Hermes, and more harnesses. Full customization of lights and actions right in your Menu Bar."
layout: page
updated: 2026-08-14
tags: [claude-code, skill, core]
---

# 💎 [camwilso/openboard](https://github.com/camwilso/openboard)

**★ 3** · exceptional (score 8.7) · Swift · ai-agents, claude, claude-code, claude-code-skill, codex-micro

> Mac App to use Codex Micro or Creator Micro with Claude, Hermes, and more harnesses. Full customization of lights and actions right in your Menu Bar.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 8.7 based on:
- ⭐ 3 stars
- 🗓️ Last pushed: 2026-08-14
- 🏷️ Tags: ai-agents, claude, claude-code, claude-code-skill, codex-micro, codex-micro-cli, hardware, macos, open, open-source, openai

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add camwilso/openboard --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/camwilso/openboard)
