---
title: "Softer-delta-999/pear-plugin — Claude Code skill discovered by cue"
description: "🍐 Connect Claude AI to iCloud to manage your Calendar, Reminders, and Contacts using natural language commands."
layout: page
updated: 2026-08-10
tags: [claude-code, skill, core]
---

# 💎 [Softer-delta-999/pear-plugin](https://github.com/Softer-delta-999/pear-plugin)

**★ 0** · exceptional (score 9.5) · anthropic, apple, calendar, carddav, claude-code

> 🍐 Connect Claude AI to iCloud to manage your Calendar, Reminders, and Contacts using natural language commands.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 9.5 based on:
- ⭐ 0 stars
- 🗓️ Last pushed: 2026-08-10
- 🏷️ Tags: anthropic, apple, calendar, carddav, claude-code, claude-skill, contacts, developer-tools, expo, hacktoberfest, icloud, mcp, openclaw, password-manager, pear, phpenv, productivity, reminders, sodium-native

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add Softer-delta-999/pear-plugin --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/Softer-delta-999/pear-plugin)
