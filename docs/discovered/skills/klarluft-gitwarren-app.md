---
title: "klarluft/gitwarren-app — Claude Code skill discovered by cue"
description: "Code review for your own machines and your own agents. Reviews live where the code does — reached over SSH, WSL or your own tailnet. Desktop app or just a brows"
layout: page
updated: 2026-09-16
tags: [claude-code, skill, frontend]
---

# 💎 [klarluft/gitwarren-app](https://github.com/klarluft/gitwarren-app)

**★ 15** · exceptional (score 10.9) · TypeScript · ai-agents, claude-code, code-review, codex, desktop-app

> Code review for your own machines and your own agents. Reviews live where the code does — reached over SSH, WSL or your own tailnet. Desktop app or just a browser tab. No server, no account.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 10.9 based on:
- ⭐ 15 stars
- 🗓️ Last pushed: 2026-09-16
- 🏷️ Tags: ai-agents, claude-code, code-review, codex, desktop-app, developer-tools, diff, electron, git, linux, local-first, macos, mcp, mcp-server, react, ssh, tailscale, typescript, windows, wsl

## Best fit cue profiles

[frontend](../frontend.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add klarluft/gitwarren-app --profile frontend
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/klarluft/gitwarren-app)
