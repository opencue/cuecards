---
title: "sohilladhani/codesynapse — Claude Code skill discovered by cue"
description: "MCP server for code intelligence - maps source code into a knowledge graph so AI tools understand architecture, not just files. Built in Rust."
layout: page
updated: 2026-07-10
tags: [claude-code, skill, core]
---

# ✨ [sohilladhani/codesynapse](https://github.com/sohilladhani/codesynapse)

**★ 5** · strong (score 7.9) · Rust · claude, code-intelligence, cursor, llm, mcp

> MCP server for code intelligence - maps source code into a knowledge graph so AI tools understand architecture, not just files. Built in Rust.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 7.9 based on:
- ⭐ 5 stars
- 🗓️ Last pushed: 2026-07-10
- 🏷️ Tags: claude, code-intelligence, cursor, llm, mcp, mcp-server, rust, tree-sitter

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add sohilladhani/codesynapse --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/sohilladhani/codesynapse)
