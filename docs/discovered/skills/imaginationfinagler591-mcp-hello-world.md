---
title: "Imaginationfinagler591/mcp-hello-world — Claude Code skill discovered by cue"
description: "Claude Code skill from Imaginationfinagler591/mcp-hello-world"
layout: page
updated: 2026-06-07
tags: [claude-code, skill, backend]
---

# ✨ [Imaginationfinagler591/mcp-hello-world](https://github.com/Imaginationfinagler591/mcp-hello-world)

**★ 0** · strong (score 7.5) · Kotlin · ai-agent, ai-integration, client-server, fastapi, fat-jar

> A Claude Code skill repository discovered by cue.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 7.5 based on:
- ⭐ 0 stars
- 🗓️ Last pushed: 2026-06-07
- 🏷️ Tags: ai-agent, ai-integration, client-server, fastapi, fat-jar, gradle, hello-world, kotlin, mcp, mcp-client, mcp-server, model-context-protocol, sdk-example, stdio

## Best fit cue profiles

[backend](../backend.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add Imaginationfinagler591/mcp-hello-world --profile backend
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/Imaginationfinagler591/mcp-hello-world)
