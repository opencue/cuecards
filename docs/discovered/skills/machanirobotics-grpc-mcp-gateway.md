---
title: "machanirobotics/grpc-mcp-gateway — Claude Code skill discovered by cue"
description: "gRPC to MCP proxy generator following the MCP Specification "
layout: page
updated: 2026-06-08
tags: [claude-code, skill, core]
---

# 💎 [machanirobotics/grpc-mcp-gateway](https://github.com/machanirobotics/grpc-mcp-gateway)

**★ 7** · exceptional (score 10.7) · Go · golang, grpc, mcp, mcp-server, protobuf

> gRPC to MCP proxy generator following the MCP Specification 

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 10.7 based on:
- ⭐ 7 stars
- 🗓️ Last pushed: 2026-06-08
- 🏷️ Tags: golang, grpc, mcp, mcp-server, protobuf, python, rust

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add machanirobotics/grpc-mcp-gateway --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/machanirobotics/grpc-mcp-gateway)
