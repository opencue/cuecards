...
Lint every `SKILL.md` in your repo against the [Claude Code skill spec](https://github.com/opencue/cuecards/blob/main/src/lib/skill-linter.ts#L1) on every push or pull request. Catches frontmatter spec violations, missing `Prerequisites`, malformed `allowed-tools`, missing trigger phrases, broken anchor links, voice-rule violations, large extractable shell blocks, missing examples, possible duplicate skills, and stale descriptions.
...
```bash
git clone --depth 1 https://github.com/opencue/cuecards ~/cue
cd ~/cue && bun install
...
MIT — same as [opencue/cuecards](https://github.com/opencue/cuecards).
