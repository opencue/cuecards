...
```bash
python3 .Codex/skills/tech-graph/scripts/generate-from-template.py <template-type> <output.svg> '<json-data>'
```

6. Validate the result:

```bash
bash .Codex/skills/tech-graph/scripts/validate-svg.sh <file.svg>
```

7. Export PNG when needed:

```bash
bash .Codex/skills/tech-graph/scripts/generate-diagram.sh -o <file.svg>
```
