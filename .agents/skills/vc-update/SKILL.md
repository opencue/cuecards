...
FILES:
  [modified]  .Codex/agents/vc-execute-agent.md  (+12 -3)
  [new]       .Codex/hooks/lib/new-util.cjs
  [removed]   .Codex/skills/deprecated-skill/SKILL.md
  [unchanged] .Codex/agents/vc-debugger.md
  ...

MERGE (preserved, manual review needed):
  [differs]   .Codex/settings.json  (+2 -1)

COPY-IF-MISSING (skipped, already present):
  [skipped]   process/context/planning/example-simple-prd.md

SYMLINKS:
  [ok]        .agents/skills -> ../.Codex/skills
  [will fix]  .codex/hooks -> ../.Codex/hooks
...
Applied:
  ...
  1 file removed
  1 symlink fixed
  1 merge file preserved (review .Codex/settings.json manually)
