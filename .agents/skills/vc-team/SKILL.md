...
Teammates with `memory: project` in their agent definition retain learnings across team sessions. Memory persists in `.Codex/agent-memory/<name>/` (gitignored). Useful for:
...
Memory persists after team cleanup -- it's in `.Codex/agent-memory/`, not `~/.Codex/teams/`.
...
If unresponsive: close terminal or kill session. Clean orphaned configs at `~/.Codex/teams/` manually.
