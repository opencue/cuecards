...
Visual verification of frontend implementations via Chrome MCP (Codex Chrome Extension) or `vc:chrome-devtools` skill fallback. Detect if frontend-related → check Chrome MCP availability → screenshot + console error check → report. Skip if not frontend.
