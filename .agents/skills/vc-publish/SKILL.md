...
9. Compare the two resolved file sets:
   ...
   - `strip` files (AGENTS.md, AGENTS.md): always flag for content review regardless of diff status.
...
FILES:
  [modified]  .Codex/agents/vc-execute-agent.md  (+8 -3)
  [modified]  .Codex/hooks/lib/scout-checker.cjs  (+2 -1)
  [new]       .Codex/skills/vc-new-skill/SKILL.md
  [strip]     AGENTS.md (needs content review)
  [strip]     AGENTS.md (needs content review)
  [unchanged] .Codex/settings.json
  ... (350 more unchanged)
...
- **Major** (2.1.0 -> 3.0.0): AGENTS.md structure changes, manifest schema changes, breaking workflow changes
...
12. On confirm:
    ...
    - **AGENTS.md and AGENTS.md stripping**: Do NOT copy the current repo's project-specific versions directly. Instead:
      1. Read the current repo's AGENTS.md/AGENTS.md.
    ...
    - Create symlinks if missing (`.agents/skills -> ../.Codex/skills`).
...
grep -ri "flowser\|tRPC\|Prisma\|Supabase\|CloakBrowser\|OpenClaw" AGENTS.md AGENTS.md
...
- AGENTS.md and AGENTS.md require special handling -- never copy the development repo's project-specific versions directly
...
See `references/vc-publish.md` for the detailed algorithm, AGENTS.md/AGENTS.md stripping rules, error handling, and example outputs.
