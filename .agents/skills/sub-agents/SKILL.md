...
Spawns external CLI AIs (Codex, cursor-agent, codex, gemini) as isolated sub-agents with dedicated context.
...
```json
{"result": "...", "exit_code": 0, "status": "success", "cli": "Codex"}
```
...
| 127 | CLI not found | Install required CLI (Codex, codex, etc.) |
...
| `--cli` | No | Force CLI: `Codex`, `cursor-agent`, `codex`, `gemini` |
