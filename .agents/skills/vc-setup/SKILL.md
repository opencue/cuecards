...
4. **Classify the project** as one of:
   ...
   - **Existing**: has `process/`, `all-context.md`, AGENTS.md with project content, or other prior context.
...
For projects that already have some form of process/ directory, context files, or AGENTS.md content.
...
- Read the existing AGENTS.md if it contains project-specific content (beyond the managed protocol).
...
2. Verify agent parity: agent names in `.Codex/agents/` should match `.codex/agents/`.
...
- **Preserve the user's existing good content.** If they have a well-written AGENTS.md, detailed context files, or a working process/ layout, merge intelligently -- do not replace with generic scans.
...
- AGENTS.md and AGENTS.md are managed protocol files. Do NOT adapt or modify them.
...
- Project-specific information (tech stack, features, conventions) belongs in `process/context/all-context.md`, not in AGENTS.md.
